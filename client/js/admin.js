...
function statusBadge(s) {
  if (s === 'active') return '<span class="badge green">🟢 آزاد</span>';
  if (s === 'limited') return '<span class="badge yellow">🟡 محدود</span>';
  if (s === 'blocked') return '<span class="badge red">🔴 مسدود</span>';
  return esc(s);
}
...
async function loadRegisterRequests() {
  try {
    ...
    tbody.innerHTML = list.map((r) => {
      const statusText =
        r.status === 'pending'
          ? '<span class="badge yellow">⏳ در انتظار</span>'
          : r.status === 'approved'
            ? '<span class="badge green">✅ تایید شده</span>'
            : '<span class="badge red">❌ رد شده</span>';
      const actions =
        r.status === 'pending'
          ? `<button class="btn-mini" data-approve="${r.id}">✅ تایید</button>
             <button class="btn-mini danger" data-reject="${r.id}">❌ رد</button>`
          : '';
      return `<tr>
...
async function loadUsers() {
  ...
  tbody.innerHTML = data.users.map((u) => {
    ...
    return `<tr>
      ...
      <td class="actions">
        <button class="btn-mini" data-edit="${u.id}">✏️</button>
        <button class="btn-mini" data-block="${u.id}">${u.status === 'blocked' ? '✅' : '🚫'}</button>
        <button class="btn-mini danger" data-del="${u.id}">🗑</button>
      </td>
    </tr>`;
  }).join('');
...
async function loadUsage() {
  ...
  $('#usage-table tbody').innerHTML = data.logs.map((l) => `<tr>
    ...
    <td>${esc(l.model || '—')}</td>
    <td>${l.status === 'ok' ? '✅' : '❌ ' + esc(l.status)}</td>
    <td>${l.duration_ms ?? '—'}</td>
